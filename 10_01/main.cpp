...
std::set<IntVec2D> find_ends(const CharGrid2D& map, const IntVec2D& current_pos, const ulong& current_height) {
    std::set<IntVec2D> ends;
    auto neighbors = get_neighbors(map, current_pos);
    for(auto n : neighbors) {
        if(n.second != '\0') {
            ulong n_h = n.second - '0';
            if(current_height == 8 && n_h == 9) ends.insert(n.first);
            else if((current_height + 1) == n_h) {
                auto other_ends = find_ends(map, n.first, n_h);
                ends.insert(other_ends.begin(), other_ends.end());
            }
        }
    }
    return ends;
...
long parse(const std::string_view& input) {
    ...
    map.iterate_over([map, &trailhead_score](const IntVec2D& pos, const char& c){
        ulong height = c - '0';
        if(height == 0) {
            auto ends = find_ends(map, pos, height);
            trailhead_score += ends.size();
        } 
    });
